...
from cat_agent.llm.schema import ASSISTANT, DEFAULT_SYSTEM_MESSAGE, USER, Message
from cat_agent.log import logger
from cat_agent.settings import (DEFAULT_MAX_REF_TOKEN, DEFAULT_PARSER_PAGE_SIZE, DEFAULT_RAG_KEYGEN_STRATEGY,
...
class Memory(Agent):
    ...
    def __init__(self,
        ...
        # Optional flag to control whether RAG indexes are rebuilt when running retrieval:
        #   rebuild_rag = True  -> always rebuild indexes from current docs
        #   rebuild_rag = False -> reuse existing indexes if present
        #   omitted / None      -> auto-detect (rebuild only when index is missing)
        self.rebuild_rag: Optional[bool] = self.cfg.get('rebuild_rag', None)
        logger.info(f"[Memory] Using rag_searchers={self.rag_searchers}")
